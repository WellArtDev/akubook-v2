...
return new class extends Migration
{
    public function up(): void
    {
        if (!Schema::hasColumn('sales_orders', 'rejected_by')) {
            Schema::table('sales_orders', function (Blueprint $table) {
                $table->foreignId('rejected_by')->nullable()->after('approved_at')->constrained('users')->onDelete('restrict');
                $table->timestamp('rejected_at')->nullable()->after('rejected_by');
                $table->text('rejection_reason')->nullable()->after('rejected_at');
                $table->foreignId('cancelled_by')->nullable()->after('rejection_reason')->constrained('users')->onDelete('restrict');
        ...
        if (DB::getDriverName() === 'sqlite') {
            DB::statement("CREATE TEMP TABLE __sales_orders_workflow_tmp AS SELECT * FROM sales_orders");
            ...
            DB::statement('INSERT INTO sales_orders SELECT id, so_number, so_date, customer_id, branch_id, customer_po_number, sales_person_id, payment_terms, delivery_terms, delivery_address_id, requested_delivery_date, notes, status, subtotal, discount_amount, tax_amount, grand_total, total_amount, approval_required, approved_by, approved_at, rejected_by, rejected_at, rejection_reason, cancelled_by, cancelled_at, cancellation_reason, credit_check_passed, credit_check_notes, created_by, created_at, updated_at, deleted_at, sales_quotation_id FROM __sales_orders_workflow_tmp');
            DB::statement('DROP TABLE __sales_orders_workflow_tmp');
    ...
    public function down(): void
    {
        Schema::table('sales_orders', function (Blueprint $table) {
            ...
            $table->dropColumn(['rejected_at', 'rejection_reason', 'cancelled_at', 'cancellation_reason']);
        });
    }
};
