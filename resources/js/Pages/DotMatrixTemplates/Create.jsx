...
export default function Create({ auth, documentTypes, defaultDocumentType, defaultFieldMap }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        columns: 80,
        rows: 66,
        margins: { top: 1, left: 2, right: 2, bottom: 1 },
    ...
    const submit = (event) => {
        ...
        post(route('dot-matrix-templates.store'));
    };
    ...
    const addField = () => {
        setData('field_map', [...data.field_map, { field: '', x: 0, y: 0, width: null, label: '' }]);
    };

    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Create Dot Matrix Template" />
            <div className="py-12">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    <div className="overflow-hidden rounded-lg bg-white shadow-sm">
                        <div className="p-6">
                            <div className="mb-6 flex items-center justify-between">
                                <h1 className="text-2xl font-semibold">Buat Dot Matrix Template</h1>
                            ...
                            <form onSubmit={submit} className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                                        {['top', 'left', 'right', 'bottom'].map((key) => (
                                            <div key={key}>
                                                <label className="mb-1 block text-xs font-medium text-slate-700 uppercase">{key}</label>
                                                <input
                                                    type="number"
                                                    value={data.margins[key]}
                                                    onChange={(e) => setData('margins', { ...data.margins, [key]: Number(e.target.value) })}
                                ...
                                <div>
                                    <div className="mb-2 flex items-center justify-between">
                                        <h2 className="text-sm font-semibold text-slate-700">Field Map</h2>
                                        <button type="button" onClick={addField} className="rounded bg-slate-200 px-3 py-1 text-xs font-semibold text-slate-700">Tambah Field</button>
                                    </div>
