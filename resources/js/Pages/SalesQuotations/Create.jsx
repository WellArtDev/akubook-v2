...
export default function Create({ auth, customers, items }) {
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Tambah Sales Quotation</h2>}>
            <Head title="Tambah Sales Quotation" />
            <div className="py-12"><div className="mx-auto max-w-7xl sm:px-6 lg:px-8"><form onSubmit={submit} className="bg-white p-6 shadow-sm sm:rounded-lg space-y-6">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <label className="block text-sm font-medium text-gray-700">Tanggal<input type="date" value={data.quotation_date} onChange={(e) => setData('quotation_date', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300" />{errors.quotation_date && <p className="text-sm text-red-600">{errors.quotation_date}</p>}</label>
                    <label className="block text-sm font-medium text-gray-700">Valid Until<input type="date" value={data.valid_until} onChange={(e) => setData('valid_until', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300" />{errors.valid_until && <p className="text-sm text-red-600">{errors.valid_until}</p>}</label>
                ...
                <div className="flex justify-end gap-2"><Link href={route('sales-quotations.index')} className="rounded bg-gray-200 px-4 py-2">Batal</Link><button disabled={processing} className="rounded bg-blue-600 px-4 py-2 text-white">Simpan</button></div>
            </form></div></div>
        </AuthenticatedLayout>
    );
}
