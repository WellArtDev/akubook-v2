...
export default function Edit({ auth, accounts, journal }) {
    const { data, setData, put, processing, errors } = useForm({
        journal_date: journal.journal_date || new Date().toISOString().split('T')[0],
    ...
    const handleSubmit = (e, submitAction) => {
        ...
        put(route('journal-entries.update', journal.id), {
                            ...
                            {journal.status !== 'draft' && (
                                <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
                                    <p className="text-sm text-yellow-800">
                                        ⚠️ Journal entry ini sudah {journal.status}. Tidak dapat diedit.
                                    </p>
                                </div>
                            )}
                            {/* Header Fields */}
