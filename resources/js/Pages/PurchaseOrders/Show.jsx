...
export default function Show({ auth, purchaseOrder }) {
    ...
    const handleSubmitForApproval = () => {
        if (confirm(`Submit Purchase Order ${purchaseOrder.po_number} untuk approval?`)) {
            router.post(route('purchase-orders.submit-approval', purchaseOrder.id));
        }
    };
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            ...
                            <div className="flex justify-end gap-2 mb-6">
                                {purchaseOrder.status === 'draft' && (
                                    <>
                                        ...
                                        {purchaseOrder.approval_required && (
                                            <button
                            ...
                            <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2">
                                <div>
                                    <h3 className="mb-4 text-lg font-semibold text-gray-900">PO Information</h3>
                                    <dl className="space-y-2">
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">PO Number:</dt>
                                            <dd className="text-sm text-gray-900">{purchaseOrder.po_number}</dd>
                                        </div>
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">PO Date:</dt>
                                            <dd className="text-sm text-gray-900">{purchaseOrder.po_date}</dd>
                                        </div>
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">Expected Delivery:</dt>
                                            <dd className="text-sm text-gray-900">{purchaseOrder.expected_delivery_date || '-'}</dd>
                                        </div>
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">Created By:</dt>
                                            <dd className="text-sm text-gray-900">{purchaseOrder.created_by?.name}</dd>
                                        </div>
                                        {purchaseOrder.approved_by && (
                                            <>
                                                <div className="flex">
                                                    <dt className="w-40 text-sm font-medium text-gray-500">Approved By:</dt>
                                                    <dd className="text-sm text-gray-900">{purchaseOrder.approved_by.name}</dd>
                                                </div>
                                                <div className="flex">
                                                    <dt className="w-40 text-sm font-medium text-gray-500">Approved At:</dt>
                                                    <dd className="text-sm text-gray-900">{purchaseOrder.approved_at}</dd>
                                                </div>
                                            </>
                                        )}
                                    </dl>
                                ...
                                <div>
                                    <h3 className="mb-4 text-lg font-semibold text-gray-900">Supplier Information</h3>
                                    <dl className="space-y-2">
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">Supplier:</dt>
                                        ...
                                        <div className="flex">
                                            ...
                                            <dd className="text-sm text-gray-900">{purchaseOrder.delivery_address?.name || '-'}</dd>
                                        </div>
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">Payment Terms:</dt>
                                            <dd className="text-sm text-gray-900">{purchaseOrder.payment_terms || '-'}</dd>
                                        </div>
                                        <div className="flex">
                                            <dt className="w-40 text-sm font-medium text-gray-500">Delivery Terms:</dt>
                                            <dd className="text-sm text-gray-900">{purchaseOrder.delivery_terms || '-'}</dd>
                                        </div>
                                    </dl>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end">
                                <div className="w-64 space-y-2">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="text-gray-900">Rp {new Intl.NumberFormat('id-ID').format(purchaseOrder.subtotal)}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span className="font-medium text-gray-500">Tax:</span>
                                        <span className="text-gray-900">Rp {new Intl.NumberFormat('id-ID').format(purchaseOrder.tax_amount)}</span>
                                    </div>
                                    <div className="flex justify-between pt-2 text-lg font-bold border-t">
                                        <span className="text-gray-900">Grand Total:</span>
                                        <span className="text-gray-900">Rp {new Intl.NumberFormat('id-ID').format(purchaseOrder.grand_total)}</span>
                                    </div>
                                </div>
                            </div>
