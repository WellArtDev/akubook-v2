...
export default function Edit({ auth, bankAccount, accounts }) {
    const { data, setData, put, processing, errors } = useForm({
        code: bankAccount.code || '', name: bankAccount.name || '', bank_name: bankAccount.bank_name || '', account_number: bankAccount.account_number || '', account_holder: bankAccount.account_holder || '', account_id: bankAccount.account_id || '', opening_balance: bankAccount.opening_balance || 0, is_active: !!bankAccount.is_active, description: bankAccount.description || '',
    });
    const submit = (e) => { e.preventDefault(); put(route('bank-accounts.update', bankAccount.id)); };

    return <Form auth={auth} title="Edit Bank Account" data={data} setData={setData} submit={submit} processing={processing} errors={errors} accounts={accounts} back={route('bank-accounts.show', bankAccount.id)} button="Update" />;
}
